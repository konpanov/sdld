...
# dots_pattern = cv2.imread("dots/0-112.jpg", cv2.IMREAD_GRAYSCALE)
# dots_pattern = cv2.imread("dots/3-52.jpg", cv2.IMREAD_GRAYSCALE)


fnames = [
    # "test/273q6x8k3ddc1.jpeg",
    # "test/9zp9si73gs2d1.jpeg", #BAD
...
for fname in fnames:
    ...
    segments = [(seg, match_shapes(seg.img, dots_pattern)) for seg in segments]
    for seg, d in [seg for seg in segments if seg[1] < .1]:
        img = draw_obj_roi(img, seg.img)
    cv2.imshow(fname, img)

    for seg, d in sorted(segments, key=lambda x:x[1]):
        cv2.imshow(f"{d}", seg.img)
        cv2.waitKey(0)
cv2.waitKey(0)
